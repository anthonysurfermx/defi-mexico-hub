...
export function ChainTVLChart({ identifier, title }: Props) {
  ...
  useEffect(() => {
    ...
    fetchChartData('chain_tvl', identifier)
      .then((d) => {
  ...
  const displayTitle = title || `${identifier.charAt(0).toUpperCase() + identifier.slice(1)} — Chain TVL`;
