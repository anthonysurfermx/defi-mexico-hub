-- ============================================
-- MEJORAS DE SEGURIDAD (ADAPTADO A ESTRUCTURA ACTUAL)
-- ============================================
...
-- ============================================
-- 2. POLÍTICAS RLS PARA STARTUPS (ADAPTADO)
-- ============================================
...
-- Ver startups aprobadas y no suspendidas (público)
CREATE POLICY "public_view_approved_startups" ON startups
  FOR SELECT
  USING (
    status = 'approved'
...
-- Admins/editores ven TODAS (incluidas suspendidas/rechazadas)
...
-- ============================================
-- 3. POLÍTICAS PARA EVENTS (ADAPTADO)
-- ============================================
...
-- Ver eventos aprobados y futuros/actuales
...
-- ============================================
-- 4. POLÍTICAS PARA COMMUNITIES (ADAPTADO)
-- ============================================
...
-- Ver comunidades aprobadas
CREATE POLICY "public_view_approved_communities" ON communities
  FOR SELECT
...
-- Sistema/triggers pueden insertar
CREATE POLICY "system_insert_notifications" ON notifications
  FOR INSERT
  WITH CHECK (true);

-- ============================================
-- 7. POLÍTICAS PARA ACTIVITY_LOG
-- ============================================
...
DROP POLICY IF EXISTS "admins_view_activity_log" ON activity_log;
DROP POLICY IF EXISTS "system_insert_activity_log" ON activity_log;
...
DROP POLICY IF EXISTS "public_view_active_profiles" ON profiles;
DROP POLICY IF EXISTS "users_update_own_profile" ON profiles;
DROP POLICY IF EXISTS "admins_manage_user_roles" ON profiles;
...
-- Usuarios pueden actualizar SOLO su perfil (sin cambiar role)
...
-- Solo admins pueden gestionar todo
...
-- ============================================
-- 10. PREVENIR ESCALACIÓN DE PRIVILEGIOS
-- ============================================
...
-- ============================================
-- 11. LOGGING DE ACCIONES ADMINISTRATIVAS
-- ============================================
...
SELECT
  tablename,
  CASE WHEN rowsecurity THEN '✅ RLS Habilitado' ELSE '❌ FALTA RLS' END as status
FROM pg_tables
...
WHERE schemaname = 'public'
  AND tablename IN (
    'profiles', 'proposals', 'startups', 'events',
    'communities', 'referents', 'notifications', 'activity_log'
  )
GROUP BY tablename
ORDER BY tablename;
